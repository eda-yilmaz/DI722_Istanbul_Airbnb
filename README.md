...
This limitation would be resolved by a DiD design using historical Inside Airbnb scrapes from 2023–2024 (see Section 8).
...
The strongest penalty is concentrated in Beşiktaş and Üsküdar cells with t-values between −4.7 and −5.0, indicating the local unlicensed discount is highly significant in these competitive areas. The 10 cells with no significant effect (grey on the map) are predominantly in outlying districts where the market is too thin for stable local estimates.
...
A key limitation of GWR is that it imposes a *single global bandwidth* across all predictor variables. This is statistically convenient but substantively restrictive: the spatial scale at which unlicensed status affects price need not be the same as the scale at which property size or availability structures price. In a geographically heterogeneous city like Istanbul — with dense competitive markets in the Bosphorus core and thin peripheral markets in outer suburbs — a single bandwidth of 101 cells may be too coarse for the city centre (where the relevant competitive neighbourhood is smaller) and simultaneously appropriate for peripheral areas. To address this, a **Multiscale Geographically Weighted Regression (MGWR)** extension was estimated (see `mgwr_analysis.py`), which allows each predictor to select its own AICc-optimal bandwidth. MGWR results are discussed in Section 7.4.

---

### 7.3 Spatio-Temporal Panel Regression

**Motivation:** The calendar data provides 12 months of daily booking information per listing. Aggregating to an H3 cell × month panel allows testing whether the compliance booking gap varies across both space and time simultaneously — a genuinely spatio-temporal analysis that neither the spatial maps nor the monthly time series could capture alone.

**Method:** For each H3 cell and each month, the mean daily booking rate was computed separately for licensed and unlicensed listings (minimum 30 listing-days per group per month to ensure stability). The compliance gap is defined as licensed − unlicensed booking rate in percentage points. The panel covers 388 cells × 13 months = **4,388 cell-month observations** with a mean gap of 11.4 pp (SD = 35.4 pp).

**Two-Way Fixed Effects Decomposition:**

A two-way FE model (`gap = α_cell + γ_month + ε`) decomposes variance in the compliance gap into its spatial, temporal, and spatio-temporal components:

| Component | R² explained | Interpretation |
|---|---|---|
| Cell fixed effects only | **0.5086** | 50.9% of gap variation is purely spatial — some cells persistently show much larger gaps than others |
| Month fixed effects only | **0.0201** | Only 2.0% is purely temporal — seasonality alone explains little |
| Two-way FE (cell + month) | **0.5286** | Together they explain 52.9% |
| Residual spatio-temporal | **0.4714** | 47.1% is genuine spatio-temporal interaction — the seasonal pattern of the gap is cell-specific |

**Monthly pattern of the compliance gap:**

| Month | Mean gap (pp) | Interpretation |
|---|---|---|
| Sep 2025 | 12.7 | Early shoulder |
| Oct 2025 | 15.0 | Pre-winter peak |
| Nov 2025 | 2.2 | Gap collapses |
| Dec 2025 | 1.0 | Minimum — winter truce |
| Jan 2026 | 9.0 | Partial recovery |
| Mar 2026 | 12.5 | Spring buildup |
| Apr 2026 | 15.8 | Spring peak |
| May 2026 | 16.3 | **Maximum gap** |
| Jun–Sep 2026 | 12.6–15.0 | Summer: high demand compresses relative gap |

| Season | Mean gap |
|---|---|
| Peak summer (Jun–Sep) | **13.8 pp** |
| Shoulder/low (Oct–May) | **10.2 pp** |

![Panel Regression](Data/fig_panel_regression.png)

👉 [Open interactive panel map (mean compliance gap per cell)](https://eda-yilmaz.github.io/DI722_Istanbul_Airbnb/Data/istanbul_panel_map.html)

**Key finding — spatio-temporal structure of the compliance gap:**

The spatial component is dominant (50.9%), confirming that which neighbourhood a listing is in matters far more than what month it is for predicting the compliance gap. However, 47.1% of gap variation is not explained by additive spatial and temporal effects — it is genuine spatio-temporal interaction. This means the seasonal pattern is heterogeneous across cells: some cells show wide seasonal swings in the gap while others are stable year-round.

The seasonal pattern itself is non-trivial. The gap collapses in November–December (1–2 pp) when overall demand is low enough that even unlicensed listings fill up, and peaks in spring (May: 16.3 pp) when guests are selective. In peak summer, high aggregate demand compresses the gap to 12–15 pp as unlicensed listings also benefit from spillover bookings. This structural pattern suggests unlicensed hosts are most vulnerable not in winter (low demand hits everyone) but in spring, when the licensed vs. unlicensed distinction most strongly shapes where demand flows.

---

### 7.4 Multiscale Geographically Weighted Regression (MGWR)
...
**Method:** MGWR was estimated using the `mgwr` library (v2.2) with `multi=True`, same cell-level features and standardised predictors as GWR. Per-variable bandwidths were selected by the same golden-section AICc criterion used for GWR.
...
### Phase 2B: Spatial Clustering of H3 Cells (pending)

K-means or DBSCAN clustering will group H3 cells into distinct market typologies using multiple cell-level attributes simultaneously: unlicensed rate, average price, booking rate, entire-home share, and availability. This would produce a data-driven map of Istanbul's STR market segments without relying on administrative boundaries.
...
├── advanced_analysis.py               # Phase 2: GWR + Panel regression (Sections 7.1–7.3)
├── mgwr_analysis.py                   # Phase 2: MGWR extension (Section 7.4)
├── Data/
...
│   ├── istanbul_panel_map.html        # Mean compliance gap per cell (Section 7.3)
│   ├── fig_license_distribution.png   # License status bar chart
...
│   ├── fig_panel_regression.png       # Panel: monthly gap + cell distribution (Section 7.3)
│   ├── fig_mgwr_comparison.png        # GWR vs MGWR side-by-side comparison (Section 7.4)
│   ├── mgwr_summary.csv               # Per-cell MGWR coefficients and significance (Section 7.4)
│   ├── mgwr_bandwidths.txt            # Per-variable optimal bandwidths (Section 7.4)
│   └── istanbul_mgwr_map.html         # Interactive MGWR local β map (Section 7.4)
```
...
| mgwr | 2.2 | Geographically Weighted Regression (GWR) |
